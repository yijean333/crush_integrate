...
class CollisionDetector(BaseDetector):
    ...
    def __init__(self, config: Dict):
        ...
        # Format: {(track_id_1, track_id_2): {'state': 'monitoring'|'confirmed', 'first_detected': frame_id, 'persist_count': int}}
        ...
        self.evasive_collision_confidence_threshold = config.get('evasive_collision_confidence_threshold', 50)  # LOWERED from 85 to 50
        ...
        for i, track_i in enumerate(tracks):
            ...
            for j, track_j in enumerate(tracks):
                ...
                if collision_detected:
                    ...
                    if self.collision_pairs[pair_key]['state'] in ['monitoring', 'confirmed']:
                        ...
                        if detection is not None:
                            ...
                            if self.collision_pairs[pair_key]['state'] == 'confirmed':
                                ...
                                logger.warning(f"[COLLISION MONITORING] Frame {current_frame}: Track {track_i.track_id} <-> Track {track_j.track_id} | "
                                             f"State: monitoring ({self.collision_pairs[pair_key]['persist_count']}/{self.persist_window})")
                else:
    ...
    def _check_pairwise_collision(self, track_i: object, track_j: object, 
        ...
        # ============================================
        # METRIC 0.5: Static Overlap Guard (CRITICAL FIX)
        # Prinsip Fisika: Collision memerlukan MOTION (v > 0)
        # Static overlap (v=0 untuk kedua track) = parking/stopped, BUKAN collision
        #
        # Validasi:
        # - Jika kedua velocity ≈ 0 → static overlap
        # - Tidak mungkin ada momentum transfer, energy loss, atau collision jika v=0
        # ============================================
        ...
        # CRITICAL: Static overlap check - RELAXED untuk allow deformation-based detection
        # Threshold: 0.5 px/fr (very low, practically stationary)
        # CHANGED: Don't early return - set flag dan allow deformation check to proceed
        STATIC_VELOCITY_THRESHOLD = 0.5
        
        # Check current frame static
        current_static = (v_i_speed < STATIC_VELOCITY_THRESHOLD and v_j_speed < STATIC_VELOCITY_THRESHOLD)
        is_static_overlap = False
        
        if current_static:
            # STABILITY CHECK: Require 3 frames of static behavior to confirm (prevent 1-frame glitches)
            history_i = getattr(track_i, 'history', [])
            history_j = getattr(track_j, 'history', [])
            
            # Helper to check historical speed
            def _check_track_static_history(history, frames=2, threshold=0.5):
                if len(history) <= frames: return True # Not enough history (assume static if current is)
                for k in range(1, frames + 1):
                    # history index: -1 is current, -2 is previous, etc.
                    # speed between -(k+1) and -k
                    prev_c = history[-(k+1)].get('center', [0,0])
                    curr_c = history[-k].get('center', [0,0])
                    spd = np.sqrt((curr_c[0]-prev_c[0])**2 + (curr_c[1]-prev_c[1])**2)
                    if spd >= threshold:
                        return False
                return True

            # Check last 4 frames (plus current frame = 5 frames total)
            # This is ~0.16s window - enough to filter out glitches like in Frame 336-339
            static_history_i = _check_track_static_history(history_i, frames=4, threshold=STATIC_VELOCITY_THRESHOLD)
            static_history_j = _check_track_static_history(history_j, frames=4, threshold=STATIC_VELOCITY_THRESHOLD)
            
            if static_history_i and static_history_j:
                is_static_overlap = True
            else:
                logger.debug(f"  STATIC STABILITY CHECK FAILED | "
                            f"Tracks {track_i.track_id} <-> {track_j.track_id} | "
                            f"Current v=0 but history indicates movement (glitch filter)")

        if is_static_overlap:
            # ============================================
            # POST-COLLISION DETECTION (ByteTrack hybrid)
            # Both tracks stationary + overlapping = possible post-collision scene
            # Check if either track had significant speed in recent history
            #
            # TIGHTENED FILTERS (v04-eval):
            # 1. Minimum history: both tracks must have >= 20 frames of history
            #    to distinguish real deceleration from bbox jitter on new tracks
            # 2. Lookback window: 30 frames (~1s) instead of 10
            # 3. Speed threshold: 8.0 px/fr (was 5.0) — filters bbox jitter
            # 4. IoU threshold: raised to 0.15-0.25 range
            # 5. Require speed DROP: max_past_speed must be >> current speed
            #    (not just "was fast once")
            # ============================================

            # Guard: skip tracks with insufficient history
            history_i = getattr(track_i, 'history', [])
            history_j = getattr(track_j, 'history', [])
            MIN_HISTORY_FOR_POST_COLLISION = 20
            if len(history_i) < MIN_HISTORY_FOR_POST_COLLISION or len(history_j) < MIN_HISTORY_FOR_POST_COLLISION:
                logger.debug(f"  POST-COLLISION SKIP | Tracks {track_i.track_id} ({len(history_i)} hist) "
                            f"<-> {track_j.track_id} ({len(history_j)} hist) | "
                            f"Need >= {MIN_HISTORY_FOR_POST_COLLISION} frames history")
            else:
                iou_static = self._compute_iou(
                    track_i.current_detection['bbox'],
                    track_j.current_detection['bbox']
                )

                # Check recent speed history — use median of top-3 speeds (not single max)
                # to filter out single-frame bbox jitter spikes
                LOOKBACK_FRAMES = 30
                past_speeds_i = []
                past_speeds_j = []

                lookback = min(LOOKBACK_FRAMES, len(history_i) - 1)
                for k in range(1, lookback + 1):
                    if k < len(history_i):
                        prev_center = history_i[-(k+1)].get('center', [0,0])
                        curr_center = history_i[-k].get('center', [0,0])
                        spd = np.sqrt((curr_center[0]-prev_center[0])**2 + (curr_center[1]-prev_center[1])**2)
                        past_speeds_i.append(spd)
                lookback_j = min(LOOKBACK_FRAMES, len(history_j) - 1)
                for k in range(1, lookback_j + 1):
                    if k < len(history_j):
                        prev_center = history_j[-(k+1)].get('center', [0,0])
                        curr_center = history_j[-k].get('center', [0,0])
                        spd = np.sqrt((curr_center[0]-prev_center[0])**2 + (curr_center[1]-prev_center[1])**2)
                        past_speeds_j.append(spd)

                # Use median of top-3 speeds to get robust "was moving" estimate
                # Single-frame spikes from bbox jitter won't pass this
                top3_i = sorted(past_speeds_i, reverse=True)[:3]
                top3_j = sorted(past_speeds_j, reverse=True)[:3]
                robust_speed_i = np.median(top3_i) if len(top3_i) >= 3 else 0.0
                robust_speed_j = np.median(top3_j) if len(top3_j) >= 3 else 0.0
                max_past_speed_i = max(past_speeds_i) if past_speeds_i else 0.0
                max_past_speed_j = max(past_speeds_j) if past_speeds_j else 0.0
                max_robust_speed = max(robust_speed_i, robust_speed_j)

                # Tightened thresholds
                POST_COLLISION_SPEED_THRESHOLD = 8.0   # was 5.0 — filters bbox jitter
                POST_COLLISION_IOU_THRESHOLD = 0.15     # was 0.05-0.15 — require meaningful overlap

                if iou_static > POST_COLLISION_IOU_THRESHOLD and max_robust_speed > POST_COLLISION_SPEED_THRESHOLD:
                    logger.warning(f"POST-COLLISION DETECTED | "
                                  f"Tracks {track_i.track_id} <-> {track_j.track_id} | "
                                  f"IoU: {iou_static:.3f} (threshold: {POST_COLLISION_IOU_THRESHOLD:.2f}) | Both now static | "
                                  f"Robust past speed: i={robust_speed_i:.1f}, j={robust_speed_j:.1f} px/fr | "
                                  f"Triggering collision detection")
                    return True, {
                        'iou': iou_static,
                        'energy_loss_i': max_past_speed_i / max(max_past_speed_i, max_past_speed_j, 1.0),
                        'energy_loss_j': max_past_speed_j / max(max_past_speed_i, max_past_speed_j, 1.0),
                        'variance_i': robust_speed_i,
                        'variance_j': robust_speed_j,
                        'detection_mode': 'post_collision_static',
                        'detection_tier': 'post_collision'
                    }

                # Only log if IoU > 0.10 to reduce spam
                if iou_static > 0.10:
                    logger.debug(f"STATIC OVERLAP | "
                                f"Tracks {track_i.track_id} <-> {track_j.track_id} | "
                                f"IoU: {iou_static:.3f} (need > {POST_COLLISION_IOU_THRESHOLD:.2f}) | "
                                f"Robust speed: {max_robust_speed:.1f} px/fr (need > {POST_COLLISION_SPEED_THRESHOLD:.1f})")

        # ============================================
        # METRIC 1: Bounding Box Overlap Violation
        # Blueprint Section 2.4.5 - Equation (509)
        # Overlap_{ij}(t) = IoU(bbox_i(t), bbox_j(t)) > 0.3
        #
        # Catatan Blueprint: Dalam traffic normal, kendaraan maintain safe distance
        # dan IoU < 0.1. IoU > 0.3 adalah anomali kuat (violation dari mutual exclusion).
        # ============================================
        ...
        # ============================================
        # UPDATE OVERLAP HISTORY (NEW - untuk temporal analysis)
        # Track berapa lama dua objek overlap untuk distinguish collision vs traffic jam
        # ============================================
        current_frame = track_i.current_detection.get('frame_id', 0)
        overlap_duration = self._update_overlap_history(track_i, track_j, iou, current_frame)
        overlap_duration_frames, overlap_state = self._check_overlap_duration(track_i, track_j)
        
        # Cleanup old overlap history periodically
        self._cleanup_overlap_history(current_frame)
        
        # ============================================
        # PROXIMITY CHECK — menggantikan hard IoU gate
        # ============================================
        # Prinsip: lewatkan pasangan yang benar-benar berjauhan, tapi TIDAK
        # memblokir kamera overhead di mana IoU bbox selalu 0.
        #
        # Pengganti scale-free: center_dist > 4.0 × max_bbox_diagonal = terlalu jauh.
        # 4.0× dipilih karena:
        #   - CE di PPL menggunakan 1.5× (kontak langsung)
        #   - KELR menggunakan 2.0× (proximity terkonfirmasi)
        #   - 4.0× = fallback untuk tier heuristic yang butuh ruang lebih lebar
        #     (rear-end collision sebelum kontak penuh, partial overlap, dsb)
        #
        # Jika IoU > 0 (kamera samping), selalu lanjut (overlap sudah terbukti dekat).
        _bbox_i_cd = track_i.current_detection['bbox']
        _bbox_j_cd = track_j.current_detection['bbox']
        _diag_i_cd = math.sqrt(max(_bbox_i_cd[2]-_bbox_i_cd[0], 1)**2 + max(_bbox_i_cd[3]-_bbox_i_cd[1], 1)**2)
        _diag_j_cd = math.sqrt(max(_bbox_j_cd[2]-_bbox_j_cd[0], 1)**2 + max(_bbox_j_cd[3]-_bbox_j_cd[1], 1)**2)
        _max_diag_cd = max(_diag_i_cd, _diag_j_cd, 1.0)
        _ci_cd = track_i.current_detection.get('center')
        _cj_cd = track_j.current_detection.get('center')
        if _ci_cd is None:
            _ci_cd = [(_bbox_i_cd[0]+_bbox_i_cd[2])/2.0, (_bbox_i_cd[1]+_bbox_i_cd[3])/2.0]
        if _cj_cd is None:
            _cj_cd = [(_bbox_j_cd[0]+_bbox_j_cd[2])/2.0, (_bbox_j_cd[1]+_bbox_j_cd[3])/2.0]
        _center_dist_cd = math.sqrt((_ci_cd[0]-_cj_cd[0])**2 + (_ci_cd[1]-_cj_cd[1])**2)
        MAX_DIST_DIAG_FACTOR = 4.0
        if iou <= 0 and _center_dist_cd > _max_diag_cd * MAX_DIST_DIAG_FACTOR:
            return False, {'iou': iou, 'energy_loss_i': 0.0, 'energy_loss_j': 0.0,
                          'variance_i': 0.0, 'variance_j': 0.0,
                          'detection_mode': 'too_far'}  # Terlalu jauh, skip tier heuristics

        # Definisi turunan untuk tier-tier fallback di bawah.
        # overlap_violation: IoU melebihi threshold normal (cocok untuk kamera samping).
        # edge_overlap: IoU partial (0.2-0.3), lebih rendah dari threshold penuh.
        # Keduanya tetap Valid untuk tier heuristic; kamera overhead akan selalu
        # mendapat edge_overlap=False dan overlap_violation=False, sehingga tier
        # fallback tidak berkontribusi — PPL (di atas) adalah satu-satunya jalur deteksi.
        overlap_violation = iou > self.iou_overlap_threshold  # 0.3
        edge_overlap = 0.2 < iou <= self.iou_overlap_threshold  # IoU partial (0.2-0.3)

        # ============================================
        # MOTORCYCLE-VEHICLE COLLISION FILTER (CRITICAL - untuk prevent false positive)
        # Untuk pasangan motorcycle-vehicle, collision HANYA boleh terdeteksi jika:
        # 1. Ada AR inversion (motorcycle jatuh) DAN
        # 2. Energy dissipation > 50% dari vehicle yang menabrak
        #
        # Filter ini mencegah false positive untuk motorcycle-vehicle pair yang hanya berdekatan
        # tanpa ada bukti motor jatuh dan energy dissipation dari vehicle yang menabrak
        # ============================================
        class_i = track_i.current_detection.get('class_name', 'unknown').lower()
        class_j = track_j.current_detection.get('class_name', 'unknown').lower()
        is_motorcycle_i = 'motorcycle' in class_i
        is_motorcycle_j = 'motorcycle' in class_j
        is_vehicle_i = self._is_vehicle_class(track_i.current_detection.get('class_name', 'unknown'))
        is_vehicle_j = self._is_vehicle_class(track_j.current_detection.get('class_name', 'unknown'))
        
        is_motorcycle_vehicle_pair = (is_motorcycle_i and is_vehicle_j) or (is_motorcycle_j and is_vehicle_i)
        
        if is_motorcycle_vehicle_pair:
            # Check AR inversion untuk motorcycle track
            ar_inversion_i, ar_past_i, ar_current_i = self._check_ar_inversion(track_i)
            ar_inversion_j, ar_past_j, ar_current_j = self._check_ar_inversion(track_j)
            ar_inversion_detected = ar_inversion_i or ar_inversion_j

            # CRITICAL: Untuk motorcycle-vehicle pair, perlu cek energy dissipation juga
            # Calculate energy loss early untuk bisa dicek di filter ini
            energy_loss_i = self._compute_energy_loss_instant(track_i, velocity_field, self.energy_loss_threshold)
            energy_loss_j = self._compute_energy_loss_instant(track_j, velocity_field, self.energy_loss_threshold)

            # Energy dissipation harus dari vehicle yang menabrak, bukan dari motorcycle yang jatuh
            tier0_6_energy = False
            if ar_inversion_i:
                # Motorcycle (track_i) jatuh, cek energy loss dari vehicle yang menabrak (track_j)
                tier0_6_energy = energy_loss_j > 0.5
            elif ar_inversion_j:
                # Motorcycle (track_j) jatuh, cek energy loss dari vehicle yang menabrak (track_i)
                tier0_6_energy = energy_loss_i > 0.5

            # RELAXED: Don't early return - set flag dan allow other tiers to check
            # Jika tidak ada AR inversion ATAU tidak ada energy dissipation > 50%, flag it
            motorcycle_strict_requirement_met = (ar_inversion_detected and tier0_6_energy)

            if not motorcycle_strict_requirement_met:
                # Flag tapi tidak reject - allow tiers lain untuk detect
                logger.warning(f"⚠️ MOTORCYCLE-VEHICLE: STRICT REQUIREMENT NOT MET (allowing other tiers) | "
                              f"Tracks {track_i.track_id} <-> {track_j.track_id} | "
                              f"IoU: {iou:.3f} | "
                              f"AR inversion: {ar_inversion_detected} | "
                              f"Energy dissipation > 50%: {tier0_6_energy} (i={energy_loss_i:.3f}, j={energy_loss_j:.3f}) | "
                              f"Proceeding with other tier checks")
        
        # ============================================
        # NORMAL TURN FILTER: Skip collision jika salah satu vehicle sedang belok normal
        # Belok normal (bukan evasive extreme) tidak boleh trigger collision detection
        # Hanya untuk vehicle-vehicle pairs (vehicle-motorcycle juga termasuk)
        # ============================================
        if is_vehicle_vehicle:
            # Check apakah track_i atau track_j sedang melakukan normal turn
            # Normal turn = angular velocity < threshold DAN lateral accel < threshold
            track_i_id = track_i.track_id
            track_j_id = track_j.track_id
            
            # Check track_i - calculate angular velocity dan lateral acceleration
            is_normal_turn_i = False
            if track_i_id in self.track_motion_history:
                history_i = self.track_motion_history[track_i_id]
                directions_i = list(history_i.get('directions', []))
                velocities_i = list(history_i.get('velocities', []))
                
                if len(directions_i) >= 2 and len(velocities_i) >= 2:
                    # Calculate angular velocity dari direction change
                    delta_theta_i = self._angle_difference(directions_i[-1], directions_i[-2])
                    angular_vel_i = abs(delta_theta_i)
                    
                    # Lateral acceleration = speed * angular_velocity (in radians)
                    speed_i = np.linalg.norm(v_i)
                    omega_rad_i = np.radians(angular_vel_i)
                    lateral_acc_i = speed_i * omega_rad_i
                    
                    # Check jika normal turn (bukan evasive extreme)
                    is_normal_turn_i = (
                        angular_vel_i < self.normal_turn_angular_velocity_max and
                        lateral_acc_i < self.normal_turn_lateral_accel_max
                    )
            
            # Check track_j - calculate angular velocity dan lateral acceleration
            is_normal_turn_j = False
            if track_j_id in self.track_motion_history:
                history_j = self.track_motion_history[track_j_id]
                directions_j = list(history_j.get('directions', []))
                velocities_j = list(history_j.get('velocities', []))
                
                if len(directions_j) >= 2 and len(velocities_j) >= 2:
                    # Calculate angular velocity dari direction change
                    delta_theta_j = self._angle_difference(directions_j[-1], directions_j[-2])
                    angular_vel_j = abs(delta_theta_j)
                    
                    # Lateral acceleration = speed * angular_velocity (in radians)
                    speed_j = np.linalg.norm(v_j)
                    omega_rad_j = np.radians(angular_vel_j)
                    lateral_acc_j = speed_j * omega_rad_j
                    
                    # Check jika normal turn (bukan evasive extreme)
                    is_normal_turn_j = (
                        angular_vel_j < self.normal_turn_angular_velocity_max and
                        lateral_acc_j < self.normal_turn_lateral_accel_max
                    )
            
            # Jika salah satu atau kedua vehicle sedang normal turn, skip collision detection
            # EXCEPTION: jangan skip jika ada bbox overlap nyata (IoU > 0) —
            # normal turn tidak menghasilkan overlap fisik; overlap = evidence of contact.
            if (is_normal_turn_i or is_normal_turn_j) and iou < 0.05:
                logger.warning(f"⏭️ SKIP COLLISION (NORMAL TURN) | "
                              f"Tracks {track_i_id} <-> {track_j_id} | "
                              f"IoU: {iou:.3f} | "
                              f"Track {track_i_id}: normal_turn={is_normal_turn_i} | "
                              f"Track {track_j_id}: normal_turn={is_normal_turn_j} | "
                              f"Normal turn detected - skipping collision detection")
                return False, {
                    'iou': iou,
                    'energy_loss_i': 0.0,
                    'energy_loss_j': 0.0,
                    'variance_i': 0.0,
                    'variance_j': 0.0,
                    'detection_mode': 'normal_turn_filter',
                    'is_normal_turn_i': is_normal_turn_i,
                    'is_normal_turn_j': is_normal_turn_j,
                    'is_normal_turn_filter': True
                }
        
        # ============================================
        # METRIC 0: Deformation Detection (Mobil Ringsek)
        # Deteksi collision berdasarkan perubahan bentuk fisik (deformasi)
        # Prinsip: Mobil yang tabrakan akan berubah bentuk (AR/area berubah)
        # Mobil yang hanya berdekatan di traffic padat: bentuk tetap sama → bukan collision
        # ============================================
        
        # ============================================
        # MULTI-TIER DETECTION STRATEGY
        # Untuk handle rear-end collision tanpa deformasi jelas
        # ============================================
        
        # Compute semua metrics terlebih dahulu
        ...
        variance_i = self._compute_acceleration_variance(track_i, velocity_field)
        variance_j = self._compute_acceleration_variance(track_j, velocity_field)
        
        # Compute rotation spike (sudden spin = collision indicator)
        ...
        rotation_spike_detected = (rotation_spike_i > self.tier1_rotation_spike_threshold or 
                                  rotation_spike_j > self.tier1_rotation_spike_threshold)
        
        # Compute push collision metrics (rear-end collision detection)
        momentum_transfer_i, momentum_transfer_j = self._compute_momentum_transfer(track_i, track_j, velocity_field)
        relative_velocity_drop = self._compute_relative_velocity_drop(track_i, track_j, velocity_field)
        push_acceleration_i, push_acceleration_j = self._compute_push_acceleration(track_i, track_j, velocity_field)
        direction_change_i, direction_change_j = self._compute_velocity_direction_change(track_i, track_j, velocity_field)
        
        # NEW: Check velocity mismatch dan approaching untuk rear-end collision
        # NOTE: Rear-end collision hanya berlaku untuk vehicle-vehicle pairs
        # Vehicle-pedestrian tidak menggunakan rear-end evidence (sudah di-filter di atas)
        velocity_mismatch, v_i_speed, v_j_speed = self._check_velocity_mismatch(track_i, track_j, velocity_field)
        is_approaching = self._check_approaching(track_i, track_j, velocity_field)
        
        # Override velocity_mismatch untuk non-vehicle-vehicle pairs (should not happen, but safety check)
        if not is_vehicle_vehicle:
            velocity_mismatch = False  # Rear-end evidence hanya untuk vehicle-vehicle
        
        # Push collision indicators
        push_momentum_detected = (momentum_transfer_i > self.momentum_transfer_threshold or 
                                momentum_transfer_j > self.momentum_transfer_threshold)
        push_relative_velocity_detected = relative_velocity_drop > self.relative_velocity_drop_threshold
        push_acceleration_detected = (push_acceleration_i > self.push_acceleration_threshold or 
                                     push_acceleration_j > self.push_acceleration_threshold)
        push_direction_change_detected = (direction_change_i > self.velocity_direction_change_threshold or 
                                          direction_change_j > self.velocity_direction_change_threshold)
        
        # ENHANCED: Combined push indicator dengan velocity mismatch dan approaching
        # Rear-end collision = (momentum/acceleration/direction change) AND (velocity mismatch OR approaching OR relative velocity drop)
        push_collision_detected = (
            (push_momentum_detected or push_acceleration_detected or push_direction_change_detected) and
            (velocity_mismatch or is_approaching or push_relative_velocity_detected)
        )

        # ── Papasan (opposite-direction) filter ──────────────────────────────────
        # Mirrors PPL A8.2 using net displacement vectors (W≈1 detik, robust terhadap jitter).
        # Physics: jika dua kendaraan bergerak dalam arah berlawanan (cos < -0.5) DAN
        # tidak ada bilateral energy loss → papasan, BUKAN tabrakan.
        # Discriminator dari head-on crash nyata: crash → kedua kendaraan decelerate bersama
        # (bilateral_energy_loss=True). Papasan → energy loss hanya satu sisi (artifact YOLO/tracker).
        _W_papasan = 30  # ≈1 detik @ 30fps, sama dengan W PPL
        _hist_i_p = getattr(track_i, 'history', [])[-_W_papasan:]
        _hist_j_p = getattr(track_j, 'history', [])[-_W_papasan:]
        _ci_p = [e['center'] for e in _hist_i_p if 'center' in e]
        _cj_p = [e['center'] for e in _hist_j_p if 'center' in e]
        _papasan_cos = 0.0
        if len(_ci_p) >= 2 and len(_cj_p) >= 2:
            _nd_i = np.array(_ci_p[-1]) - np.array(_ci_p[0])
            _nd_j = np.array(_cj_p[-1]) - np.array(_cj_p[0])
            _si_p = float(np.linalg.norm(_nd_i))
            _sj_p = float(np.linalg.norm(_nd_j))
            if _si_p > 1e-6 and _sj_p > 1e-6:
                _papasan_cos = float(np.dot(_nd_i, _nd_j)) / (_si_p * _sj_p)
        # Bilateral energy loss: KEDUA track decelerate signifikan (bukti head-on crash nyata)
        _bilateral_energy_loss = (energy_loss_i > 0.5 and energy_loss_j > 0.5)
        # is_papasan: arah berlawanan DAN tidak ada mutual deceleration → papasan, bukan tabrakan
        is_papasan = (_papasan_cos < -0.5) and not _bilateral_energy_loss
        if is_papasan:
            logger.debug(
                f"[PAPASAN FILTER] Tracks {track_i.track_id} <-> {track_j.track_id} | "
                f"cos(net_disp)={_papasan_cos:.3f} < -0.5 (berlawanan arah) | "
                f"bilateral_energy_loss={_bilateral_energy_loss} | "
                f"Suppressing rear-end evidence & deformation for tier system"
            )

        # Check deformation untuk kedua track (RAW detection)
        deformation_i, ar_change_i, area_change_i = self._compute_deformation(track_i)
        deformation_j, ar_change_j, area_change_j = self._compute_deformation(track_j)
        raw_deformation_detected = deformation_i or deformation_j

        # NOTE: Deformation validation akan dilakukan SETELAH has_rear_end_evidence didefinisikan
        # (setelah line ~750)
        
        # Track baru check
        track_i_new = len(track_i.history) < 5
        track_j_new = len(track_j.history) < 5
        
        # Get scene density untuk Tier 1.5
        scene_sparse = False
        if self.scene_analyzer is not None:
            density = getattr(self.scene_analyzer, 'current_density', 10.0)
            scene_sparse = density < self.tier1_5_sparse_density_threshold
        
        # ============================================
        # TIER 0.5: Push Collision Priority (Check FIRST - diprioritaskan untuk rear-end)
        # Kondisi: IoU > 0.25 + Energy > 0.5 + Push Collision Detected
        # Threshold lebih rendah untuk detect rear-end collision lebih agresif
        # ============================================
        tier0_5_push_iou = iou > self.tier0_5_push_iou_threshold
        tier0_5_push_energy = (energy_loss_i > self.tier0_5_push_energy_threshold or 
                              energy_loss_j > self.tier0_5_push_energy_threshold)
        
        # Push collision indicators (dengan threshold yang lebih rendah)
        # NOTE: push_momentum_detected, push_relative_velocity_detected, dll sudah dihitung di atas
        # Tidak perlu dihitung ulang di sini

        # ============================================
        # TIER 0.6: AR Inversion Collision (Motor Jatuh Setelah Ditabrak)
        # Kondisi:
        # 1. Salah satu track adalah motorcycle dengan AR inversion (AR > 1.2 → AR < 0.8)
        # 2. Energy dissipation > 50% (WAJIB - requirement dari user)
        # 3. IoU > 0.1 (ada proximity dengan track lain)
        #
        # Priority: Very High (AR inversion = strong collision evidence)
        # Logic: Diambil dari fallen_detector.py untuk detect motor jatuh
        # ============================================

        # Check AR inversion untuk kedua track
        ar_inversion_i, ar_past_i, ar_current_i = self._check_ar_inversion(track_i)
        ar_inversion_j, ar_past_j, ar_current_j = self._check_ar_inversion(track_j)
        ar_inversion_detected = ar_inversion_i or ar_inversion_j

        # AR inversion collision requirements:
        # 1. AR inversion detected (salah satu track motorcycle jatuh)
        # 2. Energy dissipation > 50% (MANDATORY) - HARUS dari vehicle yang menabrak, BUKAN dari motorcycle yang jatuh
        # 3. IoU > 0.1 (proximity dengan track lain)
        tier0_6_ar_inversion = ar_inversion_detected
        
        # CRITICAL FIX: Energy dissipation harus dari vehicle yang menabrak, bukan dari motorcycle yang jatuh
        # Jika motorcycle (track_i) yang AR inversion, maka energy loss dari vehicle (track_j) harus > 50%
        # Jika motorcycle (track_j) yang AR inversion, maka energy loss dari vehicle (track_i) harus > 50%
        tier0_6_energy = False
        if ar_inversion_i:
            # Motorcycle (track_i) jatuh, cek energy loss dari vehicle yang menabrak (track_j)
            tier0_6_energy = energy_loss_j > 0.5
        ...
        if tier0_6_ar_inversion and tier0_6_energy and tier0_6_iou:
            # Determine which track has AR inversion
            ar_inverted_track_id = track_i.track_id if ar_inversion_i else track_j.track_id
            ar_inverted_ar_past = ar_past_i if ar_inversion_i else ar_past_j
            ar_inverted_ar_current = ar_current_i if ar_inversion_i else ar_current_j
            other_track_id = track_j.track_id if ar_inversion_i else track_i.track_id

            logger.warning(f"🔥 TIER 0.6: AR INVERSION COLLISION | "
                          f"Track {ar_inverted_track_id} (MOTORCYCLE) FALLEN | "
                          f"AR: {ar_inverted_ar_past:.2f} → {ar_inverted_ar_current:.2f} | "
                          f"Collided with Track {other_track_id} | "
                          f"IoU: {iou:.3f}, Energy loss: {energy_loss_i:.3f}, {energy_loss_j:.3f}")

            debug_info = {
                'iou': iou,
                'energy_loss_i': energy_loss_i,
                'energy_loss_j': energy_loss_j,
                'ar_change_i': ar_change_i,
                'ar_change_j': ar_change_j,
                'area_change_i': area_change_i,
                'area_change_j': area_change_j,
                'variance_i': variance_i,
                'variance_j': variance_j,
                'rotation_spike_i': rotation_spike_i,
                'rotation_spike_j': rotation_spike_j,
                'ar_inversion_track': ar_inverted_track_id,
                'ar_past': ar_inverted_ar_past,
                'ar_current': ar_inverted_ar_current,
                'tier': 'ar_inversion'
            }
        ...
        # ============================================
        # TRAFFIC JAM FILTER (ENHANCED - untuk avoid false alarm di traffic padat merayap)
        # Prinsip Fisika:
        # - Traffic jam = velocity rendah konstan + PERSISTENT overlap (> 10 frames)
        # - Collision = velocity high → drop drastis + TRANSIENT overlap (< 5 frames)
        # 
        # Key Enhancement: Temporal analysis
        # - Traffic jam: overlap PERSISTENT (lama, steady-state)
        # - Collision: overlap TRANSIENT (tiba-tiba, event-based)
        # ============================================
        is_traffic_jam = self._check_traffic_jam(track_i, track_j, velocity_field, 
                                                  energy_loss_i, energy_loss_j, 
                                                  variance_i, variance_j)
        
        # ENHANCED: Tambahkan temporal persistence check
        # Jika overlap PERSISTENT (> 10 frames), pasti traffic jam, bukan collision
        # UNLESS ada strong evidence bahwa ini collision (energy loss tinggi + approaching)
        if overlap_state == 'persistent' or overlap_duration_frames >= self.persistent_overlap_min:
            # Check apakah ada strong collision evidence meskipun overlap persisten
            # STRICT: Harus ada BOTH high energy loss AND approaching/mismatch
            strong_collision_evidence = (
                (energy_loss_i > 0.8 or energy_loss_j > 0.8) and  # High energy loss
                (is_approaching or velocity_mismatch or relative_velocity_drop > 0.1)  # AND approaching/mismatch/relative v drop
            )
            
            if strong_collision_evidence:
                # Strong collision evidence - override persistent overlap
                is_traffic_jam = False
                logger.warning(f"Traffic Jam Filter: PERSISTENT overlap ({overlap_duration_frames} frames) BUT strong collision evidence "
                            f"for tracks {track_i.track_id} <-> {track_j.track_id} | "
                            f"Energy loss: {energy_loss_i:.3f}, {energy_loss_j:.3f} | "
                            f"Approaching: {is_approaching}, Mismatch: {velocity_mismatch}, Rel v Drop: {relative_velocity_drop*100:.0f}% | "
                            f"NOT filtering (likely collision)")
            else:
                # Persistent overlap tanpa strong evidence = traffic jam
                is_traffic_jam = True
                logger.warning(f"Traffic Jam Filter: PERSISTENT overlap detected ({overlap_duration_frames} frames, >={self.persistent_overlap_min}) "
                            f"for tracks {track_i.track_id} <-> {track_j.track_id} | "
                            f"Energy loss: {energy_loss_i:.3f}, {energy_loss_j:.3f} | "
                            f"Approaching: {is_approaching}, Mismatch: {velocity_mismatch}, Rel v Drop: {relative_velocity_drop*100:.0f}% | "
                            f"Filtering as TRAFFIC JAM (not collision)")
        
        # ENHANCED: Jika overlap TRANSIENT (<= 5 frames) + physics evidence, likely collision
        # TIDAK langsung filter as traffic jam even if velocity low
        # Ini untuk handle rear-end collision low-speed
        if overlap_state == 'transient':
            # Jika transient, perlu physics evidence kuat untuk declare traffic jam
            # Override traffic jam filter jika ada physics evidence (energy loss, variance) OR approaching
            if (energy_loss_i > 0.5 or energy_loss_j > 0.5 or 
                variance_i > 2.0 or variance_j > 2.0 or
                is_approaching or velocity_mismatch):
                # Ada physics evidence atau approaching - likely collision, bukan traffic jam
                is_traffic_jam = False
                logger.debug(f"Traffic Jam Filter: TRANSIENT overlap detected (<={self.transient_overlap_max} frames) "
                            f"for tracks {track_i.track_id} <-> {track_j.track_id} | "
                            f"Overlap duration: {overlap_duration_frames} frames | "
                            f"Physics evidence detected (E loss or variance or approaching) | "
                            f"NOT filtering (potential collision)")
        
        # ============================================
        # STRONG PHYSICS EVIDENCE CHECK (untuk distinguish collision vs proximity)
        # Collision = IoU tinggi + STRONG physics evidence
        # Proximity = IoU tinggi + NO strong physics evidence (cuma deket)
        # ============================================
        strong_physics_evidence = (
            energy_loss_i > 0.7 or energy_loss_j > 0.7 or  # Strong energy loss
            variance_i > 5.0 or variance_j > 5.0 or        # High variance (sudden acceleration)
            rotation_spike_i > 0.6 or rotation_spike_j > 0.6 or  # Rotation spike
            momentum_transfer_i > 20.0 or momentum_transfer_j > 20.0 or  # High momentum transfer
            push_acceleration_i > 3.0 or push_acceleration_j > 3.0  # High push acceleration
        )
        
        # ============================================
        # PROXIMITY WARNING CHECK (PRIORITY 1 - check FIRST before collision tiers)
        # LOGIC BARU: Jika hanya mobil berdekatan (IoU tinggi) TAPI:
        #   - TIDAK ada deformasi
        #   - TIDAK ada rear-end collision evidence (velocity mismatch/approaching)
        #   - TIDAK ada strong physics evidence
        # Maka ini PROXIMITY WARNING (bukan collision)
        # 
        # Collision hanya jika:
        #   - Ada deformasi ATAU
        #   - Ada rear-end collision evidence (velocity mismatch + approaching + relative v drop)
        # ============================================
        
        # Check apakah ada rear-end collision evidence
        # LOGIC STRICT: Rear-end collision jika:
        #   1. Velocity mismatch (rear vehicle lebih cepat dari front) DAN
        #   2. Ada evidence collision yang KUAT: (energy loss > 0.5 ATAU momentum transfer tinggi)
        #   3. BUKAN hanya approaching saja (bisa false positive)
        #   4. Jika relative v drop = 0% DAN push acc rendah → BUKAN collision (hanya berdekatan)
        # 
        # Catatan: 
        # - Energy loss > 0.5 sudah cukup untuk rear-end collision ringan
        # - Momentum transfer tinggi juga bisa indikator (tapi perlu hati-hati dengan kendaraan besar)
        # - Approaching TIDAK digunakan sebagai evidence sendiri (bisa false positive)
        # - Relative v drop = 0% + push acc rendah = hanya berdekatan, bukan collision
        has_strong_collision_physics = (
            (energy_loss_i > 0.5 or energy_loss_j > 0.5) or  # Energy loss > 50%
            (momentum_transfer_i > self.momentum_transfer_threshold or 
             momentum_transfer_j > self.momentum_transfer_threshold)  # Momentum transfer tinggi
        )
        
        # Check apakah ini hanya berdekatan tanpa collision physics
        # Jika relative v drop = 0% DAN push acc rendah (< 1.5) → hanya berdekatan, bukan collision
        is_weak_physics = (
            (relative_velocity_drop == 0.0) and  # Tidak ada relative velocity drop
            (push_acceleration_i < 1.5 and push_acceleration_j < 1.5)  # Push acceleration rendah (threshold dinaikkan)
        )
        
        # Rear-end collision: velocity mismatch + strong physics + NOT weak physics + NOT papasan
        # STRICT: Rear-end evidence HANYA untuk vehicle-vehicle pairs
        has_rear_end_evidence = (
            is_vehicle_vehicle and  # HANYA untuk vehicle-vehicle pairs (prevent false positive)
            velocity_mismatch and  # Rear vehicle lebih cepat dari front (indikator utama)
            has_strong_collision_physics and  # Ada evidence collision yang kuat
            not is_weak_physics and  # BUKAN hanya berdekatan tanpa collision physics
            not is_papasan  # BUKAN papasan berlawanan arah (mirror PPL A8.2)
        )

        # ============================================
        # CRITICAL FIX: DEFORMATION VALIDATION - RELAXED
        # Deformation valid jika:
        # 1. KEDUA track bergerak (v > 0.5) - real collision affects both vehicles, ATAU
        # 2. Ada rear-end evidence (one vehicle hits stationary/slow vehicle), ATAU
        # 3. Static overlap (kedua velocity < 0.5) + high IoU (> 0.3) - stationary collision
        #
        # RELAXED: Allow deformation untuk stationary object collision (pembatas, motor parkir)
        # ============================================
        both_moving = (v_i_speed > 0.5 and v_j_speed > 0.5)
        stationary_collision_valid = (is_static_overlap and iou > 0.3)  # Static + high IoU

        deformation_detected = raw_deformation_detected and (
            (both_moving and not is_papasan) or  # Exclude perspective-change artifact saat papasan
            has_rear_end_evidence or
            stationary_collision_valid
        )

        # Log jika deformation di-reject
        if raw_deformation_detected and not deformation_detected:
            _reason_deform = "papasan (perspective change)" if is_papasan else "likely perspective change or detector noise"
            logger.warning(f"⚠️ DEFORMATION REJECTED | "
                          f"Tracks {track_i.track_id} <-> {track_j.track_id} | "
                          f"v_i: {v_i_speed:.2f} px/fr, v_j: {v_j_speed:.2f} px/fr | "
                          f"Deformation: i={deformation_i}, j={deformation_j} | "
                          f"Rear-end: {has_rear_end_evidence}, Static+IoU: {stationary_collision_valid} | "
                          f"Papasan: {is_papasan} (cos={_papasan_cos:.3f}) | "
                          f"Reason: {_reason_deform}, NOT collision")

        # Check apakah ini hanya mobil berdekatan (bukan collision)
        is_just_proximity = (
            iou > 0.15 and  # IoU tinggi (berdekatan)
            not deformation_detected and  # TIDAK ada deformasi
            not has_rear_end_evidence and  # TIDAK ada rear-end evidence
            not strong_physics_evidence and  # TIDAK ada strong physics
            not is_traffic_jam  # TIDAK traffic jam
        )
        
        # ============================================
        # PROXIMITY WARNING DISABLED (commented out - not needed for now)
        # Uncomment jika ingin enable proximity warning banner
        # ============================================
        # if is_just_proximity:
        #     # Ini hanya mobil berdekatan, belum collision
        #     # Return proximity warning IMMEDIATELY (bukan collision)
        #     debug_info = {
        #         'iou': iou,
        #         'energy_loss_i': energy_loss_i,
        #         'energy_loss_j': energy_loss_j,
        ...
        #         'rotation_spike_i': rotation_spike_i,
        #         'rotation_spike_j': rotation_spike_j,
        #         'momentum_transfer_i': momentum_transfer_i,
        #         'momentum_transfer_j': momentum_transfer_j,
        #         'relative_velocity_drop': relative_velocity_drop,
        #         'push_acceleration_i': push_acceleration_i,
        #         'push_acceleration_j': push_acceleration_j,
        #         'direction_change_i': direction_change_i,
        #         'direction_change_j': direction_change_j,
        #         'velocity_mismatch': velocity_mismatch,
        #         'is_approaching': is_approaching,
        #         'v_i_speed': v_i_speed,
        #         'v_j_speed': v_j_speed,
        #         'detection_mode': 'proximity_warning',
        #         'ar_change_i': ar_change_i,
        #         'ar_change_j': ar_change_j,
        #         'area_change_i': area_change_i,
        #         'area_change_j': area_change_j,
        #         'deformation_i': deformation_i,
        #         'deformation_j': deformation_j,
        #         'tier': 0,
        #         'is_proximity_warning': True,
        #         'is_traffic_jam': is_traffic_jam,
        #         'strong_physics_evidence': strong_physics_evidence,
        #         'has_rear_end_evidence': has_rear_end_evidence
        #     }
        #     return 'proximity', debug_info
        
        # ============================================
        # COLLISION DETECTION (PRIORITY 2 - hanya jika ada evidence kuat)
        # LOGIC BARU: Collision hanya jika:
        #   1. Ada DEFORMASI (mobil ringsek) ATAU
        #   2. Ada REAR-END COLLISION EVIDENCE (velocity mismatch + approaching + relative v drop)
        # 
        # Jika hanya mobil berdekatan tanpa evidence → sudah di-filter sebagai proximity warning
        # NOTE: has_rear_end_evidence sudah didefinisikan di atas (sebelum proximity check)
        # ============================================
        
        # Collision hanya jika ada deformasi ATAU rear-end evidence
        has_collision_evidence = deformation_detected or has_rear_end_evidence
        
        # ENHANCED: Edge collision detection (IoU 0.2-0.3 dengan strong physics)
        # Untuk handle partial overlap collision (false negative fix)
        edge_collision_evidence = (
            edge_overlap and  # IoU 0.2-0.3 (partial overlap)
            not overlap_violation and  # TIDAK memenuhi normal threshold
            strong_physics_evidence  # Strong physics evidence (energy loss, variance, rotation spike)
        )
        
        # Collision evidence bisa dari: deformation, rear-end, ATAU edge collision
        has_collision_evidence = deformation_detected or has_rear_end_evidence or edge_collision_evidence
        
        # Tier 0.5: Rear-end collision priority
        # Kondisi: IoU > threshold + (deformasi ATAU rear-end evidence ATAU edge collision) + NOT Traffic Jam
        tier0_5_push_collision = (
            (tier0_5_push_iou or edge_overlap) and  # ENHANCED: Include edge overlap
            has_collision_evidence and  # STRICT: harus ada deformasi ATAU rear-end evidence ATAU edge collision
            not is_traffic_jam
        )
        
        if tier0_5_push_collision:
            detection_mode = 'tier0_5_push_priority'
            debug_info = {
                'iou': iou,
                'energy_loss_i': energy_loss_i,
                'energy_loss_j': energy_loss_j,
                ...
                'direction_change_i': direction_change_i,
                'direction_change_j': direction_change_j,
                'velocity_mismatch': velocity_mismatch,  # NEW
                'is_approaching': is_approaching,  # NEW
                'v_i_speed': v_i_speed,  # NEW
                'v_j_speed': v_j_speed,  # NEW
                'has_rear_end_evidence': has_rear_end_evidence,  # NEW
        ...
        # ============================================
        # TIER 1.5: Sparse Scene High-Confidence (Check SECOND - lebih agresif)
        # LOGIC BARU: Collision HANYA jika ada DEFORMASI ATAU REAR-END EVIDENCE ATAU EDGE COLLISION
        # Rotation spike TIDAK cukup sendiri (bisa false positive)
        # ENHANCED: Edge collision (IoU 0.2-0.3) dengan strong physics juga valid
        # ============================================
        if scene_sparse and not is_traffic_jam:  # ENHANCED: tambahkan traffic jam check
            tier1_5_iou = iou > self.tier1_5_iou_threshold
            tier1_5_energy = (energy_loss_i > self.tier1_5_energy_threshold or 
                            energy_loss_j > self.tier1_5_energy_threshold)
            
            # STRICT: Collision HANYA jika ada deformasi ATAU rear-end evidence ATAU edge collision
            tier1_5_collision_evidence = deformation_detected or has_rear_end_evidence or edge_collision_evidence
            
            tier1_5_collision = (tier1_5_iou or edge_overlap) and tier1_5_energy and tier1_5_collision_evidence
            
            if tier1_5_collision:
                detection_mode = 'tier1_5_sparse_high_confidence'
                debug_info = {
                    'iou': iou,
                    'energy_loss_i': energy_loss_i,
                    'energy_loss_j': energy_loss_j,
        ...
        # ============================================
        # TIER 1: High-Confidence Collision
        # LOGIC BARU: Collision HANYA jika ada DEFORMASI ATAU REAR-END EVIDENCE ATAU EDGE COLLISION
        # Rotation spike atau variance tinggi TIDAK cukup sendiri (bisa false positive)
        # Rotation spike tinggi (> 0.8) + deformasi/rear-end = collision
        # Variance tinggi (> 5.0) + deformasi/rear-end = collision
        # ENHANCED: Edge collision (IoU 0.2-0.3) dengan strong physics juga valid
        # ============================================
        if not is_traffic_jam:  # ENHANCED: tambahkan traffic jam check
            tier1_iou = iou > self.tier1_iou_threshold
            tier1_energy = (energy_loss_i > self.tier1_energy_threshold or 
                           energy_loss_j > self.tier1_energy_threshold)
            
            # STRICT: Collision HANYA jika ada deformasi ATAU rear-end evidence ATAU edge collision
            # Rotation spike atau variance tinggi TIDAK cukup sendiri
            tier1_collision_evidence = deformation_detected or has_rear_end_evidence or edge_collision_evidence
            
            # Collision jika: (IoU ATAU edge overlap) + Energy + (Deformasi ATAU Rear-end evidence ATAU Edge collision)
            tier1_collision = (tier1_iou or edge_overlap) and tier1_energy and tier1_collision_evidence
        else:
            tier1_collision = False  # Traffic jam detected - skip Tier 1
        
        if tier1_collision:
            # Determine detection mode berdasarkan evidence
            if deformation_detected:
                detection_mode = 'tier1_deformation'  # Deformation detected
            elif has_rear_end_evidence:
                detection_mode = 'tier1_rear_end'  # Rear-end collision evidence
            else:
                detection_mode = 'tier1_high_confidence'  # Fallback (should not happen)
        ...
        # ============================================
        # TIER 2: Deformation-Based Collision
        # Kondisi: IoU > 0.3 + Energy > 0.9 (Blueprint strict) + Deformation
        # Perlu deformation untuk avoid false positive di traffic padat
        # ENHANCED: Filter traffic jam untuk avoid false positive
        # ============================================
        if not is_traffic_jam:  # ENHANCED: tambahkan traffic jam check
            tier2_iou = iou > self.tier2_iou_threshold
            # Blueprint strict: BOTH > threshold (atau at least one untuk track baru)
            if track_i_new or track_j_new:
                tier2_energy = (energy_loss_i > self.tier2_energy_threshold or 
                               energy_loss_j > self.tier2_energy_threshold)
            else:
                tier2_energy = (energy_loss_i > self.tier2_energy_threshold and 
                               energy_loss_j > self.tier2_energy_threshold)
            
            tier2_collision = tier2_iou and tier2_energy and deformation_detected
        else:
            tier2_collision = False  # Traffic jam detected - skip Tier 2
        
        if tier2_collision:
            # Determine deformation mode
            if deformation_i and deformation_j:
                detection_mode = 'tier2_deformation_both'
            elif deformation_i:
                detection_mode = 'tier2_deformation_i'
            else:
                detection_mode = 'tier2_deformation_j'
        ...
        # ============================================
        # TIER 3: Medium-Confidence Collision (Fallback)
        # LOGIC BARU: Collision HANYA jika ada DEFORMASI ATAU REAR-END EVIDENCE ATAU EDGE COLLISION
        # Variance tinggi TIDAK cukup sendiri (bisa false positive)
        # ENHANCED: Edge collision (IoU 0.2-0.3) dengan strong physics juga valid
        # ============================================
        if not is_traffic_jam:  # ENHANCED: tambahkan traffic jam check
            tier3_iou = iou > self.tier3_iou_threshold
            tier3_energy = (energy_loss_i > self.tier3_energy_threshold or 
                           energy_loss_j > self.tier3_energy_threshold)
            
            # STRICT: Collision HANYA jika ada deformasi ATAU rear-end evidence ATAU edge collision
            tier3_collision_evidence = deformation_detected or has_rear_end_evidence or edge_collision_evidence
            
            tier3_collision = (tier3_iou or edge_overlap) and tier3_energy and tier3_collision_evidence
        else:
            tier3_collision = False  # Traffic jam detected - skip Tier 3
        
        if tier3_collision:
            detection_mode = 'tier3_medium_confidence'
            debug_info = {
                'iou': iou,
                'energy_loss_i': energy_loss_i,
                'energy_loss_j': energy_loss_j,
        ...
        # ============================================
        # TIER FALLBACK: EVIDENCE-BASED COMBINATION (NEW - UNIVERSAL DETECTION)
        # Jika semua tier gagal, coba evidence-based combination
        # Collision detected jika >= 2 dari evidence berikut terpenuhi:
        # 1. IoU > 0.2 (overlap detected)
        # 2. Energy loss > 0.3 (moderate energy dissipation)
        # 3. Deformation detected (bbox shape change)
        # 4. Momentum transfer > 10 (momentum exchange)
        # 5. High variance (> 3.0) (sudden acceleration)
        # 6. Static overlap + high IoU (stationary collision)
        #
        # ADAPTIVE THRESHOLDS: Relax thresholds untuk universal detection
        # ============================================
        if not is_traffic_jam:
            # ── Papasan filter (Traffic-pass suppressor) ────────────────────────
            # Masalah: bus/kendaraan besar melintas (papasan) di samping kendaraan
            # lambat/berhenti → IoU overlap terjadi karena ukuran bbox besar →
            # fallback ter-trigger meski bukan collision.
            #
            # Solusi: hitung sudut antara dua vektor kecepatan. Jika cos_angle < -0.5
            # (sudut > 120°, keduanya bergerak ke arah berlawanan), ini adalah papasan,
            # BUKAN tabrakan → skip seluruh fallback tier.
            #
            # Ini sama persis dengan Sub-guard A8.2 di PPL.
            # Catatan: filter ini hanya aktif jika KEDUA track bergerak cukup cepat
            # (≥ 1.0 px/fr) — kendaraan statis tidak dihitung sebagai 'papasan'.
            # Gunakan is_papasan yang sudah dihitung di atas (net displacement, lebih robust
            # dari instantaneous velocity yang dipakai sebelumnya di sini).
            # is_papasan = cos(net_disp_i, net_disp_j) < -0.5 AND NOT bilateral_energy_loss
            if is_papasan:
                return False, {'iou': iou, 'energy_loss_i': energy_loss_i,
                               'energy_loss_j': energy_loss_j, 'variance_i': variance_i,
                               'variance_j': variance_j, 'detection_mode': 'papasan_filtered'}


            # ── Proximity guard: jika kedua bbox tidak overlap (IoU=0), pastikan
            #    pasangan ini memang dekat secara fisik sebelum evaluasi evidence.
            #    Ini mencegah false positive untuk kendaraan yang jauh namun
            #    kebetulan mengalami deceleration / high variance secara bersamaan.
            _fb_prox_ok = True
            if iou <= 0:
                _bbox_i_fb = track_i.current_detection.get('bbox') if track_i.current_detection else None
                _bbox_j_fb = track_j.current_detection.get('bbox') if track_j.current_detection else None
                if _bbox_i_fb is not None and _bbox_j_fb is not None:
                    _di_fb = math.sqrt((_bbox_i_fb[2]-_bbox_i_fb[0])**2 + (_bbox_i_fb[3]-_bbox_i_fb[1])**2)
                    _dj_fb = math.sqrt((_bbox_j_fb[2]-_bbox_j_fb[0])**2 + (_bbox_j_fb[3]-_bbox_j_fb[1])**2)
                    _max_d_fb = max(_di_fb, _dj_fb, 1.0)
                    _ci_fb = [(_bbox_i_fb[0]+_bbox_i_fb[2])/2.0, (_bbox_i_fb[1]+_bbox_i_fb[3])/2.0]
                    _cj_fb = [(_bbox_j_fb[0]+_bbox_j_fb[2])/2.0, (_bbox_j_fb[1]+_bbox_j_fb[3])/2.0]
                    _dist_fb = math.sqrt((_ci_fb[0]-_cj_fb[0])**2 + (_ci_fb[1]-_cj_fb[1])**2)
                    # Fallback hanya aktif jika jarak ≤ 1.5× diagonal bbox terbesar
                    # (lebih ketat dari 2.0× untuk menghindari ripple effect FP)
                    if _dist_fb > _max_d_fb * 1.5:
                        _fb_prox_ok = False

            evidence_list = []

            # Evidence 1: IoU overlap (RELAXED threshold)
            if iou > 0.2:
                evidence_list.append('iou')

            # Evidence 2: Energy loss (RELAXED threshold - 30%)
            if energy_loss_i > 0.3 or energy_loss_j > 0.3:
                evidence_list.append('energy')

            # Evidence 3: Deformation (bbox shape change)
            if deformation_detected:
                evidence_list.append('deformation')

            # Evidence 4: Momentum transfer — DINONAKTIFKAN
            # momentum = bbox_area(px²) × delta_v(px/fr) → nilai jutaan, threshold=10 selalu terpenuhi
            # Momentum tidak reliable sebagai evidence → dihapus dari FALLBACK
            # (momentum masih dihitung untuk debug_info, tidak dipakai sebagai evidence)

            # Evidence 5: High variance (sudden acceleration) — dengan sanity cap
            # Nilai > 50 px/frame² = jitter tracker/YOLO, bukan fisika nyata kendaraan
            _VAR_JITTER_CAP = 50.0
            if (3.0 < variance_i <= _VAR_JITTER_CAP) or (3.0 < variance_j <= _VAR_JITTER_CAP):
                evidence_list.append('variance')

            # Evidence 6: Static overlap + high IoU (stationary collision)
            if is_static_overlap and iou > 0.3:
                evidence_list.append('static_collision')

            # Evidence 7: Rotation spike (sudden spin)
            if rotation_spike_i > 0.4 or rotation_spike_j > 0.4:
                evidence_list.append('rotation')

            # DECISION — 2-tier berdasarkan IoU overlap:
            #
            # Tier A — IoU ≥ 0.20 (real overlap, kamera samping/isometric):
            #   Cukup 2 evidence. Bounding box benar-benar tumpang tindih.
            #
            # Tier B — IoU 0.12–0.20 (partial/edge overlap):
            #   Butuh 3 evidence + proximity guard.
            #   (raised from 0.05 — IoU < 0.12 = tangential brush, bukan tabrakan)
            #
            # Tier C — IoU < 0.05 (kamera overhead / YOLO jitter):
            #   DINONAKTIFKAN. PPL adalah satu-satunya detektor untuk kasus ini.
            #   EVIDENCE_BASED_FALLBACK tidak pernah dirancang untuk IoU~0 —
            #   evidence (energy, variance) terlalu mudah terpenuhi oleh ripple effect
            #   collision sehingga menghasilkan spam FP ke semua pasangan di sekitar.
            evidence_count = len(evidence_list)
            if iou >= 0.20:
                # Tier A: genuine overlap
                fallback_collision = _fb_prox_ok and (evidence_count >= 2)
            elif iou >= 0.12:
                # Tier B: partial/edge overlap (was 0.05 — raised to 0.12 to cut FPs)
                # IoU 0.05–0.12 adalah "tangential brush" / drift bbox YOLO, bukan tabrakan nyata
                fallback_collision = _fb_prox_ok and (evidence_count >= 3)
            else:
                # Tier C: DISABLED — biarkan PPL yang handle overhead camera
                fallback_collision = False

            if fallback_collision:
                logger.warning(f"🔄 TIER FALLBACK: EVIDENCE-BASED DETECTION | "
                              f"Tracks {track_i.track_id} <-> {track_j.track_id} | "
                              f"Evidence count: {evidence_count}/7 | "
                              f"Evidence: {', '.join(evidence_list)} | "
                              f"IoU: {iou:.3f}, Energy: {max(energy_loss_i, energy_loss_j):.3f}, "
                              f"Deformation: {deformation_detected}")
                ...
                debug_info = {
                    ...
                    'v_j_speed': v_j_speed,
                    'has_rear_end_evidence': has_rear_end_evidence,
                    'detection_mode': 'evidence_based_fallback',
                    'ar_change_i': ar_change_i,
                    'ar_change_j': ar_change_j,
                    'area_change_i': area_change_i,
                    'area_change_j': area_change_j,
        ...
        # Jika sampai di sini, berarti tidak ada tier yang match dan bukan proximity warning
    ...
    def _check_velocity_mismatch(self, track_i: object, track_j: object,
        ...
        if rel_pos_norm < 1e-6:
            ...
            is_mismatch = abs(v_i_speed - v_j_speed) > 1.0  # Significant speed difference
        else:
            ...
            # Velocity mismatch: i is approaching j (v_i_proj > 0) and faster than j
            # OR j is moving away from i (v_j_proj < 0) which means i is catching up
            is_mismatch = (v_i_proj > 0.5 and v_i_speed > v_j_speed) or \
    ...
    def _detect_evasive_maneuver(self, track_id: int, current_velocity: np.ndarray) -> Tuple[bool, float, float, float, float]:
        ...
        # Compare last 2-3 frames
        if len(directions) >= 3 and len(velocities) >= 3:
            # Recent direction change (last 2 frames)
            delta_theta_recent = self._angle_difference(directions[-1], directions[-2])
            delta_frames = frames[-1] - frames[-2]

            if delta_frames > 0:
                angular_velocity = abs(delta_theta_recent) / delta_frames
            else:
        ...
        for track in tracks:
            ...
            if is_evasive:
                ...
                has_collision_evidence = (
                    (sudden_dec_pct > 50.0 and direction_change > 90.0 and past_speed >= 1.0) or
